...
#ifdef REDOOMED
    macroSwizzleInstanceMethod(MapView, rdePNGDataAtScale:, rdeGSPatch_rdePNGDataAtScale:);
#else // Original
...
@implementation MapView (RDE_PPGNUstepGlue_BitmapNonpremultipliedPNG)

- (NSData *) rdeGSPatch_rdePNGDataAtScale: (float) pngScale
{
    NSData *returnedData;

    gAllowInPlaceUnpremultiply = YES;

    returnedData = [self rdeGSPatch_rdePNGDataAtScale: pngScale];
...
- (bool) ppGSGlue_Premultiply
{
    ...
    if (!(_format & NSAlphaNonpremultipliedBitmapFormat)
        || ![self ppGSGlue_CanPremultiplyImportedBitmap])
#else // Original
    if (!(_format & NSAlphaNonpremultipliedBitmapFormat)
...
#ifdef REDOOMED
    bitmapSize = NSMakeSize([self pixelsWide], [self pixelsHigh]);
#else // Original
