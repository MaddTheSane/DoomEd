...
NSStream *NXMapFile(const char *filepath, int mode)
{
    // DoomEd only calls NXMapFile() with readonly mode
    return [NSInputStream inputStreamWithFileAtPath: RDE_NSStringFromCString(filepath)];
}

void NXCloseMemory(NSStream *stream, int option)
{
    [stream close];
}

void NXGetMemoryBuffer(NSStream *stream, char **streambuf, int *len, int *maxLen)
{
    NSUInteger nsUIntLen;

    [(NSInputStream *) stream getBuffer: (unsigned char **) streambuf length: &nsUIntLen];

    *maxLen = *len = (int) nsUIntLen;
}
