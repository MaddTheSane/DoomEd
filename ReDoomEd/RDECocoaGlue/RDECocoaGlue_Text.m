...
#import "RDECocoaGlue_Text.h"


@implementation NSText (RDECocoaGlue_TextMethods)
...
- (id) setSel: (int) start : (int) end
{
    NSRange selectedRange = {0,0};

    if (start < [self textLength])
    {
        selectedRange = NSMakeRange(start, end - start + 1);
    }
...
- (id) replaceSel: (const char *) aString
{
    NSRange selectedRange = [self selectedRange];
    NSString *replacementString = RDE_NSStringFromCString(aString);

    if (!selectedRange.length)
    {
        selectedRange.location = [self textLength];
    }

    [self replaceCharactersInRange: selectedRange
            withString: replacementString];

    selectedRange.length = [replacementString length];
