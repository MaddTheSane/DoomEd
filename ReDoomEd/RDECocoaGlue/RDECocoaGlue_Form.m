...
#import "RDECocoaGlue_Form.h"


@implementation NSForm (RDECocoaGlue_FormMethods)

- (int) intValueAt: (int) index
{
    NSFormCell *formCell = [self cellAtIndex: index];

    return [formCell intValue];
}

- (id) setIntValue: (int) intValue at: (int) index
{
    NSFormCell *formCell = [self cellAtIndex: index];

    [formCell setIntValue: intValue];
...
- (const char *) stringValueAt: (int) index
{
    ...
    return RDE_CStringFromNSString([formCell stringValue]);
}

- (id) setStringValue: (const char *) stringValue at: (int) index
{
    NSFormCell *formCell = [self cellAtIndex: index];

    [formCell setStringValue: RDE_NSStringFromCString(stringValue)];
