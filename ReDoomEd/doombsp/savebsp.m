...
static void RDE_FreeBSPNode(bspnode_t *bspnode)
{
	...
	RDE_FreeBSPNode(bspnode->side[0]);
	RDE_FreeBSPNode(bspnode->side[1]);

	[bspnode->lines_i release];
...
void ProcessNodes (void)
{
	...
	[subsecstore_i autorelease];
	[maplinestore_i autorelease];
	[nodestore_i autorelease];

	// prevent memory leaks: free startnode global allocated in BuildBSP()
	RDE_FreeBSPNode(startnode);
#endif
}
...
#ifdef REDOOMED
	// prevent memory leaks
	[mapvertexstore_i autorelease];
	[ldefstore_i autorelease];
	[sdefstore_i autorelease];
#endif
}
